...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof. intros. rewrite H , H0. reflexivity.
(** [] *)
...
Proof. intros b c H. destruct c. reflexivity. rewrite <- H. destruct b. reflexivity. reflexivity. Qed.
(** [] *)

(** **** Exercise: 1 star (zero_nbeq_plus_1)  *)
Theorem zero_nbeq_plus_1 : forall n : nat,
  beq_nat 0 (n + 1) = false.
Proof. intros. destruct n. reflexivity. reflexivity. Qed.
(** [] *)
...
Theorem andb_eq_orb :
  ...
  b = c.
Proof. intros.
assert (false || c = c). reflexivity.
assert (true && c = c). reflexivity.
assert (true || c = true). reflexivity.
assert (false && c = false). reflexivity.
destruct b.
...
Fixpoint bin_to_nat (n : bin) : nat :=
  ...
  | succt n' => 1 + 2 * bin_to_nat n'
  end.

Example test_bin_incr1 : bin_to_nat (incr zero) = S O.
Proof. reflexivity. Qed.
Example test_bin_incr2 : bin_to_nat (incr (incr zero)) = S (S O).
Proof. reflexivity. Qed.
Example test_bin_incr3 : bin_to_nat (incr (incr (incr zero))) = S (S (S O)).
Proof. reflexivity. Qed.
Example test_bin_incr4 : bin_to_nat (incr (incr (incr (incr zero)))) = S (S (S (S O))).
Proof. reflexivity. Qed.
Example test_bin_incr5 : bin_to_nat (incr (incr (incr (incr (incr zero))))) = S (S (S (S (S O)))).
Proof. reflexivity. Qed.

Theorem plus_O_r : forall n : nat , n + 0 = n.
Proof. intros. induction n. reflexivity. simpl. rewrite IHn. reflexivity. Qed.

Theorem incr_eq_S : forall n : bin, bin_to_nat (incr n) = S (bin_to_nat n).
...
rewrite IHn.
simpl.
assert (forall a b : nat, a + S b = S (a + b)).
{ intros. induction a. trivial. simpl. rewrite IHa. reflexivity. }
rewrite H.
reflexivity.
Qed.
(** [] *)
